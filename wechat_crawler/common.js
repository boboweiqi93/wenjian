    ...
    article_urls.forEach(function (article_url, index) {
      ...
      if (index < 10) {
        task1.push(function (callback) {
          var article_object = {
            title: '', abstract: "", thumb: "",url: '', read_num: '', like_num: '',
          ...
          task2.push(function (callback) {
            request(article_url, function (err, response, html) {
              ...
              //简要说明
              var abstract = $(".weui_media_desc").text();
              //缩略图
              // var thumb = $(".weui_media_hd").css('background-image');
                // thumb = thumb.replace(/.*\s?url\([\'\"]?/, '').replace(/[\'\"]?\).*/, '')
