...
Ut.search_wechat = function (public_num, callback) {
  ...
  request(url, function (err, response, html) {
    ...
    //现在得到的wechat_num ，在浏览器上打不开，这也是他的加密方式，就是把所有的"amp;"替换为空，并且因为async.waterfall这个函数是顺序进行，并且下面的函数会用到上面函数的处理结果，所以上面函数的处理结果要利用回调函数传递给下面的函数使用，所以要写在callback中。
    setTimeout(function () {
...
Ut.look_wechat_by_url = function (url, callback) {
  ...
  request(url, function (err, response, html) {
    ...
    task4.push(function (html, callback) {
      ...
      //得到这个对象的第二个内容，因为这个内容中有我们想要的文章消息；
      msglist = msglist[1];
      //replace(/(&quot;)/g, '\\\"') 这一步是有的时候有用，有的时候没用，处理意外情况的
        //后面那个是将所有的空格都替换为空
    ...
    async.waterfall(task4, function (err, result) {
      ...
      setTimeout(function () {
        callback(null, article_titles, article_urls, article_pub_times,article_thumb,article_abstract);
      }, 1000 + Math.ceil(Math.random() * 500));
    })
  })
};
...
// */
Ut.get_info_by_url = function (article_titles, article_urls, article_pub_times,article_thumb,article_abstract, callback) {
  var task1 = [];
  var articles = [];
  if (article_urls.length > 0) {
    article_urls.forEach(function (article_url, index) {
      //此if可以限制获取文章的数量,限制抓取10篇
      if (index < 15) {
        task1.push(function (callback) {
          var article_object = {
            title: '', abstract: "", platform: "", source: "", thumb: "", content: "",url: '', tags: "",
            time: '', state: "", read_num: '', like_num: ''
          }
          var task2 = [];
          //发布日期,作者,公众号,url
          task2.push(function (callback) {
            request(article_url, function (err, response, html) {
              ...
              var $ = cheerio.load(html);
              //作者
              // var source = $($(".rich_media_meta_list em")[1]).text();
              var source = $("#post-user").text()

              article_object.title = article_titles[index].replace(/amp;/g, '').replace(/&quot;/g, '"');
              article_object.abstract = article_abstract[index];
              article_object.platform = "微信公众号";
              article_object.source = source;
              article_object.thumb = article_thumb[index];
              article_object.content = "";
              article_object.url = article_urls[index];
              article_object.tags = "";
              article_object.time = article_pub_times[index];
              article_object.state = 1;
  ...
  task_code.push(function (base64, callback) {
    ...
    var opts = {
      ...
      headers: {
        ...
        //授权码
        "Authorization": "APPCODE e046f6efe7934914bc0107a34ff333cf"
      }
    };
